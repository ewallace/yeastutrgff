...
### List of verified nuclear ORF names only

```{r verified_ORF_names,dependson="load_gffs"}
verified_nuclear_ORF_names <- 
    gff_CDS %>%
...
## Make transcripts which are longest isoforms if we have data, fixed-length if we don't
...
## Make transcripts which are abundant isoforms if we have data, fixed-length if we don't
...
## Verified ORFs only: Make transcripts which are abundant isoforms if we have data, fixed-length if we don't

```{r gff_verified_abundant_or_CDSext,dependson=c("verified_ORF_names","gff_abundant_or_CDSext")}
gff_verified_abundant_or_CDSext <- 
    bind_rows(gff_abundtxs  %>% 
                  filter( Gene %in% verified_nuclear_ORF_names),
              gff_verCDSext %>% 
                  filter(
                     Gene %in% genes_verified_no_abundanttx)
              ) %>% 
...
outfile_verified_abundant_or_CDSext <- here::here(
    "out",
...
write_lines(
    x=c("##gff-version 3", 
        "##source-version yeastutrgff",
        paste("##date",lubridate::today())),
    path=outfile_verified_abundant_or_CDSext)

write_gff(x=gff_verified_abundant_or_CDSext,
          path=outfile_verified_abundant_or_CDSext,
          append=TRUE)
```
