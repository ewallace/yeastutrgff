...
```{r gff_collected,dependson=c("load_gffs","gff_introns","load_gff_5UTRs_nointrons","gff_3UTRs")}
...
# gff_collected_mRNA %>% filter(Gene=="YFL039C") %>% replace_parent
...
gff_collected <- bind_rows(gff_SGD_chromosomes_ncRNA,
                           gff_collected_mRNA) %>%
    dplyr::arrange(Seqid,Start,desc(End))

outfile_gff_collected <-
    here::here("out","gff_Chromosomes_ncRNAs_verifiedmRNAparts.gff")
write_lines(
    x=c("##gff-version 3", 
        "##source-version yeastutrgff",
        paste("##date",lubridate::today()),
...
outfile_gff_collected_nochr <-
    here::here("out","gff_ncRNAs_verifiedmRNAparts.gff")
write_lines(
    x=c("##gff-version 3", 
        "##source-version yeastutrgff",
        paste("##date",lubridate::today()),
        "##selected features: ncRNA, CDS, 5'UTRs, 3'UTRs, introns"),
    path=outfile_gff_collected_nochr)
write_gff(gff_collected %>% filter(Type != "chromosome"),
          path=outfile_gff_collected_nochr,
          append=TRUE)
...
```{r gff_collected_featureCounts_workaround,dependson="gff_collected"}
outfile_gff_collected_nochr_workaround <-
    here::here("out","gff_ncRNAs_verifiedmRNAparts_typeworkaround.gff")
write_lines(
    x=c("##gff-version 3", 
        "##source-version yeastutrgff",
        paste("##date",lubridate::today()),
    ...
    path=outfile_gff_collected_nochr_workaround)
write_gff(gff_collected %>% 
...
```{r txgff_collected,dependson=c("gff_collected","txgff_functions")}
txpartlength_mRNA <- 
    gff_collected %>%
...
write_lines(
    ...
    path=outfile_txgff_collected)
write_gff(txgff_collected,
          path=outfile_txgff_collected,
          append=TRUE)
```
...
```{r rv_testregion_YBL72ish,dependson="gff_collected"}
gff_testregion_YBL72ish <- gff_collected %>%
    filter(Seqid=="chrII",
           Start > 45000, 
           End < 89500)
